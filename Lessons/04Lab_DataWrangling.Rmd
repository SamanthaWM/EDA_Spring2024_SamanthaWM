...
NTL.phys.data.PeterPaul <- 
  filter(NTL.phys.data, lakename %in% c("Paul Lake", "Peter Lake"))   #, % is the "and" symbol
...
NTL.phys.data.processed <- 
  NTL.phys.data %>%
  filter(lakename == "Paul Lake" | lakename == "Peter Lake") %>%          #, | is the "or" symbol  # 'filter" Filters rows
  select(lakename, sampledate:temperature_C) %>%     # "select" Filters columns
  mutate(temperature_F = (temperature_C*9/5) + 32)

#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 
# through 1999 only for July.

NTL.phys.data.exercise3 <-
  NTL.phys.data %>%
...
NTL.phys.data.exercise5 <-
  NTL.phys.data %>%
  pivot_longer(
    cols = (irradianceWater_irradianceDeck),
